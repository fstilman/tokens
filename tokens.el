...
it is saved to the kill ring.  If MATCH-NUMBER is ommited it
defaults to 1."
  (interactive
  ...
  (save-excursion
    ...
    (let ((match-number (or match-number 1)))
      (when (tokens--search token-type match-number)
    ...
    (let ((match-number (or match-number 1)))
      (catch 'found
        (dolist (token-type (tokens--token-list))
          (when (tokens-kill-ring-save token-type match-number)
...
(defun tokens--search (token-type match-number)
  "Search the current line for the Nth MATCH-NUMBER of TOKEN-TYPE."
...
If 'tokens-highlight-match' is non-nil and `pulse' is available,
the saved region is highlight.  Also, if `tokens-verbose-match' is
non-nil, the saved region is shown on the echo area.

Return saved region as a string."
  ...
  (let ((token (buffer-substring-no-properties region-start region-end)))
    (when (eq tokens-verbose-match t)
      (message "Saved: %s"token))
    token))
